...
impl MountPoint {
    ...
    fn add_link_name(&mut self, name: &[u16]) {
        if self.dos_name.is_empty() && is_dos_name(&name) {
...
#[allow(dead_code)]
mod mountmgr {
    use winapi::shared::minwindef::{USHORT, ULONG, DWORD};
    use winapi::um::winioctl::{METHOD_BUFFERED, FILE_READ_ACCESS, FILE_WRITE_ACCESS, FILE_ANY_ACCESS};
    use winapi::STRUCT;

    pub static DEVICE_NAME: &[u16] = wstr!("\\??\\MountPointManager");

    pub const MOUNTMGRCONTROLTYPE: DWORD = 0x0000006D; // 'm'
    pub const MOUNTDEVCONTROLTYPE: DWORD = 0x0000004D; // 'M'
...
impl MountManager {
    ...
    pub fn query_all(&self) -> Result<Vec<MountPoint>> {
        unsafe {
            let zeroed: mountmgr::MOUNTMGR_MOUNT_POINT = mem::zeroed();
            let mut out_buffer = alloc_buffer(512);
            loop {
                let (status, _) = self.0.ioctl_raw(mountmgr::IOCTL_MOUNTMGR_QUERY_POINTS, as_byte_slice(&zeroed), &mut out_buffer);
                match status {
            ...
            let mut point_map: BTreeMap<&[u8],MountPoint> = BTreeMap::new();
            for index in 0..raw_points.NumberOfMountPoints {
                ...
                let entry = point_map.entry(id_bytes).or_insert_with(|| MountPoint::new(id_bytes));
